...
const filters = ["All", "Internship", "Research", "Volunteer", "Scholarship", "Job"];

export default function OpportunitiesScreen() {
  const { opportunities, savedIds, appliedIds, toggleSaveOpportunity } = useLaunchpad();
  const [query, setQuery] = useState("");
  const [activeFilter, setActiveFilter] = useState("All");

  const filteredItems = useMemo(() => {
    return opportunities.filter((item) => {
      const passesType = activeFilter === "All" ? true : item.type === activeFilter;
    ...
    <Screen>
      <View className="pt-2">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-6">
          {filters.map((filter) => (
            <Pressable key={filter} onPress={() => setActiveFilter(filter)} className="mr-3">
              <Tag
                label={filter}
